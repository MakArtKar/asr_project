...
class TorchDeepSpeech(BaselineModel):
    ...
    def __init__(
        ...
        n_feature = n_feats
        super(TorchDeepSpeech, self).__init__(n_feats, n_class)
    ...
    def forward(self, spectrogram, **batch) -> torch.Tensor:
        """
        Args:
            spectrogram (torch.Tensor): Tensor of dimension (batch, feature, time).
        Returns:
            Tensor: Predictor tensor of dimension (batch, time, class).
        """
        # N x F x T
        x = spectrogram.transpose(1, 2)[:, None, ...]
        # N x C x T x F
        x = self.fc1(x)
        # N x C x T x H
        x = self.fc2(x)
